...
def search_term_list(context, data_dict):
    toolkit.check_access('sysadmin', context, data_dict)
    offset = data_dict.get('start',0)
    connection = model.Session.connection()
    your_query="""
    SELECT count(*) from discovery_searchterm
    """
    the_count = connection.execute(str(your_query)).scalar()
    res = connection.execute("select id,term,count from discovery_searchterm order by id limit 100 offset "+str(offset)+" rows").fetchall()
    term_list = []
    for id, term, count in res:
        term_list.append({'id':id, 'term':term, 'count':count})
    return {'total':the_count,'search_terms':term_list}

def search_term_delete(context, data_dict):
    toolkit.check_access('sysadmin', context, data_dict)
    term_id = _get_or_bust(data_dict, 'id')
    from ckan.model.meta import Session
    from .model import SearchTerm
    try:
        for term in Session.query(SearchTerm).filter(SearchTerm.id == term_id):
            session = model.Session()
            session.execute("delete from discovery_searchterm where id = "+str(term.id))
            session.commit()
            return "success"
    except:
        return "not found"
